...
export default function GamePage() {
    ...
    const [gameStatus, setGameStatus] = useState('พร้อมลุยหรือยัง? กดปุ่มเพื่อเริ่ม!');
    ...
    }, [currentPosition]); // re-draw if pos changes? No, only on resize really, but updatePlayerUI needs currentPosition
    ...
    const movePlayer = async (steps) => {
        ...
        setGameStatus(`เดิน ${steps} ช่อง... ไปที่ ${destination}`);
    ...
    const checkSpecialTiles = (pos) => {
        ...
        if (ladder) {
            setGameStatus("เจอไต่บันได! ขึ้นไปเลย!");
            setCurrentPosition(ladder.end);
        }

        const snake = SNAKES.find(s => s.start === pos);
        if (snake) {
            setGameStatus("โอ้ย! เจองูฉก ตกลงไปข้างล่าง!");
            setCurrentPosition(snake.end);
        }

        if (pos === BOARD_SIZE || (ladder && ladder.end === BOARD_SIZE)) {
            // If ladder leads to win, or normal move leads to win
            const finalPos = ladder ? ladder.end : pos;
        ...
        setIsMoving(false);
        if (movesLeft <= 0 && pos !== BOARD_SIZE) { // Check movesLeft (it was decremented before roll)
            // Actually movesLeft is already decremented. 
            // If we just used the last move and didn't win:
            if (movesLeft - 1 < 0) { // Wait, logic: decremented at start. So if 0 now.
                // We need to check if we can roll again.
                // movesLeft is the value *before* this turn? No, I updated it `setMovesLeft(prev => prev - 1)`.
                // But inside this function closure `movesLeft` is stale.
                // Rely on state effect? Or functional check.
                // Actually React state updates are scheduled.
                // Let's rely on `useEffect` or just trust the logic:
                // We can check `movesLeft` via a ref or just assume the UI handles the disable button.
                // But we need to update message.
                // It's tricky to get the *new* movesLeft here without a Ref.
                // Simplified: The UI button disables when movesLeft === 0.
                // So here just set status.
            }
        }

        // Update End game message if runs out
        // We can do this in a useEffect on [movesLeft, isMoving]
    };

    // Check Game Over (Out of moves)
    useEffect(() => {
        if (!isMoving && movesLeft === 0 && currentPosition !== BOARD_SIZE && !showWinModal) {
            setGameStatus("จบเกม! คุณใช้สิทธิ์ทอยครบแล้ว");
        }
    ...
    // Logic from game.js:
    // Row 8 (Top): 43..48 (Left->Right? No. Zigzag)
    // Let's replicate strict Grid rendering.
    // We render 48 divs.
    // We need to know which number goes to which slot.
    // Slot 0 (Top-Left) -> Slot 47 (Bottom-Right).
    // Total 48 slots.
            ...
            <div className="container mx-auto max-w-5xl flex flex-col md:flex-row gap-4 md:gap-8 p-4 md:p-8 flex-grow justify-start md:justify-center items-center md:items-start">
                ...
                >
                    ...
                    <div id="game-board" ref={boardRef} className="grid grid-cols-6 gap-1 w-full h-full p-1 relative z-10">
                        {cells.map((num) => {
                            const isLadder = LADDERS.some(l => l.start === num || l.end === num);
                            const isSnake = SNAKES.some(s => s.start === num || s.end === num);

                            let cellColorClass = 'bg-white text-slate-400';
                            if (isLadder) cellColorClass = 'bg-green-300 text-green-800 font-bold';
                            else if (isSnake) cellColorClass = 'bg-red-300 text-red-800 font-bold';
                            else if (num % 2 === 0) cellColorClass = 'bg-emerald-50/50 text-slate-400';
